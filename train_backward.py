...
train = datasets.tuple_dataset.TupleDataset(bold_train, img_train)
validation = datasets.tuple_dataset.TupleDataset(bold_val, img_val)
...
# Reconstructs images in the validation set at the end of training
...
trainer.extend(Reconstructor(recon_train_iter, model, shape=[128,128], filename=out_dir + '/recon_train'), trigger=(nepochs, 'epoch'))
trainer.extend(Reconstructor(recon_validation_iter, model, shape=[128,128], filename=out_dir + '/recon_validation'), trigger=(nepochs, 'epoch'))
