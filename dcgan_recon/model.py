...
from sequential.functions import Activation, dropout, gaussian_noise, circularmask, circularmaskgpu, tanh, sigmoid, reshape, reshape_1d, squeeze
...
# data
use_gpu = False if args.gpu_device == -1 else True  # for training the GAN
image_width = args.image_dims
...
if os.path.isfile(discriminator_sequence_filename): 
    ...
    config.use_feature_matching = True    # changed to improve gan
...
if os.path.isfile(generator_sequence_filename):   
    ...
    input_size = get_in_size_of_deconv_layers(image_width, num_layers=5, ksize=4, stride=2)
    ...
    if args.apply_kaymask:
        # apply kaymask.
        if use_gpu:  # sequential doesn't pass arguments for some reason
            print "Using GPU and kaymask_gpu"
            generator.add(circularmaskgpu(loadmat('kaymask64.mat')['kaymask64'].astype('float32')))
        else:
            generator.add(circularmask(loadmat('kaymask64.mat')['kaymask64'].astype('float32')))
...
if args.gpu_device != -1 and use_gpu:
    cuda.get_device_from_id(args.gpu_device).use()
    gan.to_gpu()
