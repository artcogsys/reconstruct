...
from dataset import load_gray_images
...
def sample_from_data(images, batchsize):
    ...
    #x_batch = np.empty((batchsize, 3, height, width), dtype=np.float32)  # for RGB images
    x_batch = np.empty((batchsize, 1, height, width), dtype=np.float32)      # for L images  TODO maybe use two dimensions, maybe this prevents low level artefacts
...
def main():
    ...
    if debug: 
        images = load_gray_images("/vol/ccnlab-scratch1/katmul/reconv/gentrain_imgs/dim64x64bwnomask_debug/") # dim64x64glove_debug/")
    else:
        images = load_gray_images("/vol/ccnlab-scratch1/katmul/reconv/gentrain_imgs/dim64x64bwnomask/")    #dim64x64glove/")
    ...
    # settings
    max_epoch = 1000    # 75 (brains) 500 (vim1) 1000 (reconstruction was good at 500)
    ...
    for epoch in xrange(1, max_epoch + 1):
        ...
        for t in xrange(num_updates_per_epoch):
            ...
            x_fake.unchain_backward()  # TODO: x_fake has trailing dimension
            ...
            # TODO select low-level discriminator vs. deep discriminator at random
            # to avoid low-level artifacts
