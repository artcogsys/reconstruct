...
from sklearn.preprocessing import normalize   # simple normalization function for big ndarrays
...
class GAN():
    ...
    def sample_z(self, batchsize=1):
        ...
        # tanh uniform
        #z_batch = np.tanh(1.5*np.random.uniform(-1, 1, (batchsize, ndim_z))).astype(np.float32)
        #########z_batch = np.random.normal(0, 1, (batchsize, ndim_z)).astype(np.float32)
        z_batch = np.random.uniform(-1,1, (batchsize, ndim_z)).astype(np.float32)
        #z_batch = z_batch / np.linalg.norm(z_batch, axis=1) # what about norm == 0
        #z_batch = normalize(z_batch, norm='l2')
        z_batch = F.normalize(z_batch).data   #normalize(z_batch, norm='l2')
        return z_batch

    def generate_x(self, batchsize=1, test=False, as_numpy=False, return_z = False):
        if not return_z:
            return self.generate_x_from_z(self.sample_z(batchsize), test=test, as_numpy=as_numpy)
        else:
            z_batch = self.sample_z(batchsize)
            return self.generate_x_from_z(z_batch, test=test, as_numpy=as_numpy), z_batch
    ...
    def load(self, dir=None):
        print "Loading trained GAN from ", dir
        if dir is None:
            raise Exception()
        self.generator.load(dir + "latest_generator.hdf5")
        self.discriminator.load(dir + "latest_discriminator.hdf5")

    def save(self, dir=None, epoch_id=None):
        if dir is None:
            raise Exception()
        try:
            os.mkdir(dir)
        ...
        self.generator.save(dir + "latest_generator.hdf5")
        self.discriminator.save(dir + "latest_discriminator.hdf5")

        if epoch_id != None and epoch_id % 30 == 0: 
            self.generator.save(dir + str(epoch_id) + "ep_generator.hdf5")
            self.discriminator.save(dir + str(epoch_id) + "ep_discriminator.hdf5")
