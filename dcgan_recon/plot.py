...
def tile_rgb_images(x, dir=None, filename="x", row=10, col=10):
	...
	for m in range(row * col):
		...
		pylab.imshow(np.clip(np.squeeze(x[m]), 0, 1), cmap=plt.get_cmap('gray'), interpolation="none")
		pylab.axis("off")
...
def plot(filename="gen"):
	...
	#tile_rgb_images(x_fake.transpose(0, 2, 3, 1), dir=args.model_dir, filename=filename)
	tile_rgb_images(x_fake, dir=args.model_dir, filename=filename)
...
def sample_from_data(images, batchsize):
	...
	x_batch = np.empty((batchsize, 1, height, width), dtype=np.float32)
...
def plot_original_data(filename="data"):
	...
	tile_rgb_images(x_true, dir=args.model_dir, filename=filename)
