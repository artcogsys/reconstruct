...
class CrossEntropy(function.Function):
    ...
    def forward(self, inputs):
        ...
        loss = -xp.sum(
            self.ignore_mask * (t * xp.log(x + 1e-5) +   # TODO: is this correct?
                                (1.0-t) * xp.log1p(-x + 1e-5)) )
        # original is the tensorflow formulation: 
    ...
    def backward(self, inputs, grad_outputs):
        ...
        #y, = sigmoid.Sigmoid(self.use_cudnn).forward((x,))  # removed
        gx = xp.divide(
            gloss * self.ignore_mask * (x - t), self.count,
            dtype=x.dtype)
        return gx, None


def cross_entropy(x, t, use_cudnn=True, normalize=True):
    ...
    """
    return CrossEntropy(use_cudnn, normalize)(x, t)
