...
# TODO should be one function, pass numerical framework (cp or np) and use variable xp as elsewhere in code
class circularmask(Function):
	def __init__(self, mask=None, on_gpu=False, offset = (137./255. * 2.0) - 1.0):
		self._function = "circularmask"
		
		if mask != None: 
			self.mask = np.array(mask).astype('float32')
		else:
			self.mask = None
	...
	def __call__(self, x):
		return x * F.broadcast_to(self.mask, x.shape) + self.offset


class circularmaskgpu(Function):
	def __init__(self, mask=None, offset = (137./255. * 2.0) - 1.0):
		self._function = "circularmaskgpu"

		if mask != None: 
			self.mask = cp.array(mask).astype('float32')
		else:
			self.mask = None
	...
	def __call__(self, x):
		return x * cp.array(F.broadcast_to(self.mask, x.shape).data) + self.offset
